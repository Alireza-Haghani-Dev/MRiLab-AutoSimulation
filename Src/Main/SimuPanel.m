...
% pathstr='LoadBrainS_uimenu_Callback(hObject, eventdata, handles)';
% 
%  gui_State.gui_Callback = str2func(pathstr);
%  gui_mainfcn(gui_State, varargin{:});
% End initialization code - DO NOT EDIT

% allProperties = get(handles.Batch_pushbutton);

% Print the properties to the Command Window
...
disp('output');
 ...
 %SeqList_pushbutton_Callback(hObject, eventdata, handles);
...
% 
 if countoo==1 
     Mag_uipushtool_ClickedCallback(hObject, eventdata, handles);
      
 
 elseif countoo==2
MagList_pushbutton_Callback(hObject, eventdata, handles);
 elseif countoo==3

   SeqList_pushbutton_Callback(hObject, eventdata, handles);
 elseif countoo==4
      CoilList_pushbutton_Callback(hObject, eventdata, handles);
 elseif countoo==5

    Scan_pushbutton_Callback(handles.Scan_pushbutton, eventdata, handles);

 else 
     disp('countoo is wrong');
...
guidata(hObject, handles);
LoadVObjXML_uimenu_Callback(hObject, eventdata, handles);
...
%View_pushbutton_Callback(hObject, eventdata, handles);
...
%     return;
% end
% DoUpdateInfo(handles,'Virtual object XML file was successfully loaded1.');
...
try
    ...
    filename = 'VObj_Cylindrical.xml';
    pathname = '.\Config\VObj\VObj_EFMRI\VObj_Cylindrical';
    handles.VObjXMLFile = fullfile(pathname, filename); % Combine path and file

    % Call the function to load the XML file into the design panel
    %VObjDesignPanel(handles);
...
% Provide feedback about the successful loading of the file
DoUpdateInfo(handles, 'Virtual object XML file was successfully loaded2.');
set(handles.LoadVObjXML_uimenu, 'Label', ['Load Phantom XML : ' filename]);
set(handles.LoadVObjfromXML_uimenu, 'Enable', 'on');
LoadVObjfromXML_uimenu_Callback(hObject, eventdata, handles);
